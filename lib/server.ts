...
export const config=()=>{const e=env as any;return {url:String(e.SUPABASE_URL||'').replace(/\/$/,''),key:String(e.SUPABASE_ANON_KEY||''),domain:String(e.SCHOOL_EMAIL_DOMAIN||'unlv.nevada.edu').toLowerCase(),admins:String(e.ADMIN_EMAILS||'').toLowerCase().split(',').map((s:string)=>s.trim())};};
...
export async function identity(req:Request){
 ...
 const res=await fetch(cfg.url+'/auth/v1/user',{headers:{apikey:cfg.key,Authorization:'Bearer '+token}});if(!res.ok)return null;const u=await res.json() as any;if(!u.email_confirmed_at||!u.email||u.email.toLowerCase().split('@')[1]!==cfg.domain)return null;return {id:u.id as string,email:u.email.toLowerCase() as string,expiresAt:Math.min(payload.exp,payload.iat+14400)*1000};}catch{return null;}
}
