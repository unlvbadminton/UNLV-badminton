...
The administrator allowlist grants an exact-email exception to the student domain check for both OTP requests and session validation. It does not allow other `@unlv.edu` addresses. OTP proof is still required. An explicitly empty `ADMIN_EMAILS` disables bootstrap defaults. Runtime configuration, SMTP delivery and database migration still require completion before real login is ready.
